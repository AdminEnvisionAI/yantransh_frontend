...
// Image mapping
const imageMap = {
  "cloud-infrastructure.png": new URL("../assets/cloud-infrastructure.png", import.meta.url).href,
};

const CloudInfrastructure = () => {
  const page = contentData.pages?.["cloud-infrastructure"] || {};
    ...
    <div style={{ minHeight: "100vh", background: T.white }}>
      ...
      <section style={{ position: "relative", overflow: "hidden", background: T.navy, paddingTop: 100 }}>
        ...
        <W style={{ position: "relative", zIndex: 2, paddingTop: 80, paddingBottom: 60 }}>
          <Rv>
            <a href={page.backLink || "/#services"} style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "rgba(255,255,255,0.7)", fontFamily: T.fn, fontSize: 13, textDecoration: "none", marginBottom: 24 }}>
              <span style={{ transform: "rotate(180deg)", display: "inline-flex" }}>{IC.arrow}</span> {page.backLabel || "Back to Services"}
            </a>
          </Rv>
          <Rv d={0.1}>
            <span style={{ fontFamily: T.fn, fontSize: 13, fontWeight: 600, color: T.blue, letterSpacing: 1, textTransform: "uppercase" }}>{page.category || "Services"}</span>
          </Rv>
          <Rv d={0.15}>
            <h1 style={{ fontFamily: T.fd, fontSize: "clamp(32px, 5vw, 52px)", fontWeight: 700, color: T.white, lineHeight: 1.2, margin: "16px 0 20px" }}>{page.title || "Cloud & Infrastructure"}</h1>
          </Rv>
          <Rv d={0.2}>
      ...
      <section style={{ padding: "70px 0", background: T.white }}>
        <W>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 40, alignItems: "center", marginBottom: 50 }}>
            <Rv>
              <img src={img} alt={page.title || "Cloud & Infrastructure"} style={{ width: "100%", height: "auto", borderRadius: T.r, boxShadow: "0 10px 40px rgba(0,0,0,0.1)" }} />
            </Rv>
            <Rv d={0.1}>
