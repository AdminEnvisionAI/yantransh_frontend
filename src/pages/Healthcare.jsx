...
// Image mapping
const imageMap = {
  "healthcare.png": new URL("../assets/healthcare.png", import.meta.url).href,
};

const Healthcare = () => {
  const page = contentData.pages?.healthcare || {};
    ...
    <div style={{ minHeight: "100vh", background: T.white }}>
      ...
      <section style={{ position: "relative", overflow: "hidden", background: T.navy, paddingTop: 100 }}>
        ...
        <W style={{ position: "relative", zIndex: 2, paddingTop: 80, paddingBottom: 60 }}>
          ...
          <Rv d={0.15}>
            <h1 style={{ fontFamily: T.fd, fontSize: "clamp(32px, 5vw, 52px)", fontWeight: 700, color: T.white, lineHeight: 1.2, margin: "16px 0 20px" }}>{page.title || "Healthcare"}</h1>
          </Rv>
          <Rv d={0.2}>
      ...
      <section style={{ padding: "70px 0", background: T.white }}>
        <W>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 40, alignItems: "center", marginBottom: 50 }}>
            <Rv>
              <img src={img} alt={page.title || "Healthcare"} style={{ width: "100%", height: "auto", borderRadius: T.r, boxShadow: "0 10px 40px rgba(0,0,0,0.1)" }} />
            </Rv>
            <Rv d={0.1}>
